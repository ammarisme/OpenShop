...
using RetailShop.Areas.Administrators.Models;
using RetailShop.DAL;

namespace RetailShop.Areas.Administrators.Controllers
{
 ...
 */
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<ActionResult> Register(RegisterAdministratorViewModel model)
        {
            ApplicationDbContext appDb = new ApplicationDbContext();

            if (ModelState.IsValid)
            {
                // create a administratortable entry and a user table entry
                var administrator = new Administrator { UserName = model.Email, Email = model.Email};
                var result = await UManager.CreateAsync(administrator, model.Password);
                if (result.Succeeded)
                {
                    var addedToRole = await UManager.AddToRoleAsync(administrator.Id, "Administrator");
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    var isAdministrator = await UManager.IsInRoleAsync(user.Id, "Administrator");
                    if (isAdministrator)
                    {
        ...
        public ActionResult Manage()
        {
            ...
            ManageAdministratorViewModel model = new ManageAdministratorViewModel { Id = User.Identity.GetUserId(), Email = administrator.Email};
